...
const SecteursSection = () => {
  return (
    <section id="secteurs" className="bg-blanc-casse py-16 md:py-24">
        ...
        <div className="relative overflow-hidden mb-4">
          ...
          <div className="flex gap-4 animate-marquee-reverse md:animate-marquee-reverse-slow hover:[animation-play-state:paused]">
            {[...domains, ...domains].map((d, i) => (
              <div
                key={i}
                className="card-lift flex items-center gap-3 px-5 py-3 rounded-full border border-navy/10 bg-card shadow-card hover:bg-white hover:border-or-mat/30 shrink-0"
              >
                <div className="w-9 h-9 rounded-full bg-or-mat/10 flex items-center justify-center shrink-0">
                  <d.icon size={18} className="text-or-mat-clair" />
                </div>
                <span className="font-dm text-sm font-medium text-ardoise whitespace-nowrap">{d.label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="relative overflow-hidden mb-4">
          ...
          <div className="flex gap-4 animate-marquee md:animate-marquee-slow hover:[animation-play-state:paused]">
            {[...sectors, ...sectors].map((s, i) => (
              ...
              >
                <div className="w-9 h-9 rounded-full bg-or-mat/10 flex items-center justify-center shrink-0">
                  <s.icon size={18} className="text-or-mat-clair" />
                </div>
                <span className="font-dm text-sm font-medium text-ardoise whitespace-nowrap">{s.label}</span>
              </div>
            ))}
          </div>
        </div>
